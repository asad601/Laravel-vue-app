  ...
  methods: {
    uploadPhoto(item) {
      if (this.editedItem.photo != null) {
        ...
        axios
          .post("http:/laravue/public/api/user/photo", formData)
          .then(res => {
    ...
    updateRole(item) {
      const index = this.users.data.indexOf(item);
      axios
        .post("http:/laravue/public/api/user/role", { role: item.role, user: item.id })
        .then(res => {
          this.text =
    ...
    checkEmail() {
      if (/.+@.+\..+/.test(this.editedItem.email)) {
        axios
          .post("http:/laravue/public/api/email/verify", { email: this.editedItem.email })
          .then(res => {
    ...
    deleteAll() {
      ...
      if (decide) {
        axios
          .post("http:/laravue/public/api/users/delete", { users: this.selected })
          .then(res => {
    ...
    searchIt(e) {
      if (e.length > 3) {
        axios
          .get(`http:/laravue/public/api/users/${e}`)
          .then(res => (this.users = res.data.users))
          .catch(err => console.dir(err.response));
      }
      if (e.length <= 0) {
        axios
          .get(`http:/laravue/public/api/users`)
          .then(res => (this.users = res.data.users))
          .catch(err => console.dir(err.response));
      }
    },
    paginate(e) {
      ...
      const orderBy =
        this.options.sortDesc.length > 0 && this.options.sortDesc[0]
          ...
          : "desc";
      axios
        .get(`http:/laravue/public/api/users?page=${e.page}`, {
          params: {
    ...
    deleteItem(item) {
      ...
      if (decide) {
        axios
          .delete("http:/laravue/public/api/users/" + item.id)
          .then(res => {
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        axios
          .put("http:/laravue/public/api/users/" + this.editedItem.id, this.editedItem)
          .then(res => {
        ...
        axios
          .post("http:/laravue/public/api/users", this.editedItem)
          .then(res => {
